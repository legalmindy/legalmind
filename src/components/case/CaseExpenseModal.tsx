...
export function CaseExpenseModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-900/40 p-0 sm:items-center sm:p-4" dir="rtl">
      <button type="button" className="absolute inset-0 cursor-default" aria-label="إغلاق" onClick={onClose} />
      <div className="relative z-10 max-h-[92vh] w-full max-w-lg overflow-y-auto rounded-t-2xl border border-slate-100 bg-white shadow-xl sm:rounded-2xl">
        <div className="sticky top-0 flex items-center justify-between border-b border-slate-100 bg-white px-4 py-3">
          <h2 className="text-sm font-black text-slate-900">{initial ? 'تعديل مصروف' : 'إضافة مصروف'}</h2>
        ...
        <form onSubmit={(e) => void handleSubmit(e)} className="space-y-3 p-4">
          ...
          <div className="flex gap-2 pt-1">
            <button
              type="submit"
              disabled={saving}
              className="flex-1 rounded-xl bg-[#7A1F2B] px-4 py-2.5 text-sm font-bold text-white disabled:opacity-60"
            >
              {saving ? 'جاري الحفظ…' : initial ? 'حفظ التعديلات' : 'حفظ المصروف'}
            </button>
